...
# Define the list of object classes that should trigger an error
error_classes = ["cell phone", "mobile", "laptop"]
...
while True:
    ...
    # Loop through each pose detection result
    ...
    object_results = object_model.predict(source=frame, show=False)  # Set show=False to not display the result frame automatically

    # Extract the results for object detection
    object_result = object_results[0]
    
    # Variable to track if any error class is detected
    error_detected = False
    
    # Draw bounding boxes on the frame and check for error classes
    for box in object_result.boxes:
        x1, y1, x2, y2, conf, cls = box.xyxy[0].tolist() + [box.conf[0], box.cls[0]]
        class_name = object_model.names[int(cls)]
        
        # Check if the detected class is in the error list
        if class_name.lower() in [name.lower() for name in error_classes]:
            error_detected = True
    ...
    # Display an error message if any error class is detected
    if error_detected:
        cv2.putText(frame, "Error: Object Detected", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2)
